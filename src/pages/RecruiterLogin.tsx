...
const RecruiterLogin = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if ((recruiterId === 'REC001' || recruiterId === 'REC002') && password === 'testpass123') {
        localStorage.setItem('userRole', 'recruiter');
        localStorage.setItem('currentUserId', recruiterId);
        localStorage.setItem('recruiterLoginStatus', true.toString());
        navigate('/recruiter/panel');
      } else {
        toast({
          title: "Login Failed",
          description: "Invalid recruiter ID or password.",
          variant: "destructive",
        });
      }
      setIsLoading(false);
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="REC001"
                value={recruiterId}
                onChange={(e) => setRecruiterId(e.target.value.toUpperCase())}
                required
              />
            </div>
            <div className="space-y-2">
